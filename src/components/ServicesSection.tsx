...
const services = [
  {
    num: "SVC // 01",
    title: "Job Costing ★ CORE",
    body: "Every project gets a complete P&L — labor, materials, equipment, subcontractors, TPA fees, and overhead. Every month, without exception.",
    tag: "Core Differentiator",
    core: true,
  },
  {
    num: "SVC // 02",
    title: "Supplement Tracking ★ CORE",
    body: "Every supplement tracked from submission through carrier approval to final payment. Monthly recovery report showing what was collected and what's still outstanding.",
    tag: "Industry Exclusive",
    core: true,
  },
  {
    num: "SVC // 03",
    title: "Estimate-to-Books Mapping",
    body: "We bridge the gap between your estimating platform and your accounting software. Xactimate, Albi, Dash, JobNimbus — all mapped correctly into your books.",
    tag: null,
    core: false,
  },
  {
    num: "SVC // 04",
    title: "TPA Channel AR",
    body: "Contractor Connection, Alacrity, Sedgwick, Code Blue — each tracked on its own aging schedule. Know exactly who owes what, by program, and for how long.",
    tag: null,
    core: false,
  },
  {
    num: "SVC // 05",
    title: "Sub & 1099 Management",
    body: "W-9 collection, COI tracking, sub costs coded to jobs, and 1099 filing handled. Full audit trail from dispatch to payment, year-round — not just in January.",
    tag: null,
    core: false,
  },
  {
    num: "SVC // 06",
    title: "Equipment Revenue & Asset Tracking",
    body: "Drying equipment is a daily billable line item. We reconcile equipment-day logs against invoices so no billable day disappears. Depreciation schedules maintained separately.",
    tag: null,
    core: false,
  },
  {
    ...
    body: "Clean close by the 15th, every month. P&L, balance sheet, job profitability summary, supplement recovery report. CPA-ready. Always.",
    tag: null,
    core: false,
  },
  {
    ...
    body: "If your books are a Category 3 situation — and most are when we start — we remediate first. Full chart-of-accounts rebuild, transaction reclassification, missed supplement recovery.",
    tag: null,
    core: false,
  },
  {
    num: "SVC // 09",
    title: "QBO + ERP Setup & Migration",
    body: "QuickBooks Online and Desktop, Sage 100, Acumatica, Foundation. We set up, migrate, and maintain your accounting platform — properly configured for restoration job costing from day one.",
    tag: null,
    core: false,
  },
];
...
const expenseSteps = [
  ...
  { n: "04", t: "Routes to the correct job cost code automatically — no data entry required", final: false },
  {
    ...
    t: (
      <>
        Job P&L reflects actual costs{" "}
        <strong className="text-c3-yellow font-medium">daily</strong> — margin
    ...
    >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-[2px] bg-c3-border mt-10">
        {services.map(({ num, title, body, tag, core }) => (
          <div
            key={num}
            className={`bg-c3-card border-t-[2px] p-7 transition-all duration-[220ms] ${
              core
          ...
          >
            ...
            <h4
              className={`font-cond font-bold text-[18px] uppercase tracking-[0.04em] mt-[10px] leading-[1.2] ${
                core ? "text-c3-yellow" : "text-c3-text"
              }`}
            >
              {title}
            </h4>
            <p className="font-body font-light text-[13px] text-c3-mid leading-[1.7] mt-[10px]">
              {body}
            </p>
            {tag && (
              <span
                className={`inline-block mt-[14px] font-mono text-[9px] tracking-[0.2em] border py-1 px-[7px] uppercase ${
                  core
                    ? "border-c3-yellow-dim text-c3-yellow"
                    : "border-c3-b2 text-c3-dim"
                }`}
              >
                {tag}
              </span>
      ...
      >
        <div>
          ...
          <p className="font-body font-light text-[14px] text-c3-mid mt-[14px] leading-[1.85]">
            ...
            transaction routes directly to the correct job cost code in real
            time.
          </p>
          <p className="font-body font-light text-[14px] text-c3-mid mt-[14px] leading-[1.85]">
        ...
        <div className="flex flex-col gap-[10px]">
          {expenseSteps.map(({ n, t, final: isFinal }) => (
            <div
              key={n}
              className={`bg-c3-card border-l-[2px] py-[14px] px-[18px] flex gap-[14px] items-center transition-colors duration-200 hover:border-l-c3-yellow ${
                isFinal ? "border-c3-yellow" : "border-c3-yellow-dim"
              }`}
            >
              <div className="font-mono text-[10px] text-c3-yellow tracking-[0.2em] shrink-0">
                {n}
              </div>
              <div
                className={`font-body text-[13.5px] leading-[1.5] ${
                  isFinal ? "text-c3-text" : "text-c3-mid"
                }`}
              >
                {t}
              </div>
      ...
      <div className="mt-9 py-5 px-6 border border-c3-border bg-[rgba(255,255,255,0.018)] grid grid-cols-[24px_1fr] gap-[18px] items-start">
        ...
        <div>
          ...
          <p className="font-body font-light text-[13.5px] text-c3-mid leading-[1.8] mt-2">
            ...
            <Link
              href="#pricing"
              className="text-c3-yellow border-b border-c3-yellow-dim hover:border-c3-yellow transition-colors duration-200"
            >
